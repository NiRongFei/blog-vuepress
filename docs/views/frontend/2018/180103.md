...
`Flow` 是 JavaScript 的静态类型检查工具，该库的目标在于检查 JavaScript 中的类型错误，开发者通常不需要修改代码即可使用，故使用成本很低。同时，它也提供额外语法支持，使得开发者能更大程度地发挥 `Flow` 的作用。总结一句话：`Flow` 将 Javascript 从弱类型语言变成了强类型语言。
